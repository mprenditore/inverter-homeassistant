    ...
    while (true) {
        ...
        if (ups_qmod_changed && ups_qpiri_changed && ups_qpigs_changed) {
            ...
            if (reply1 && reply2 && warnings) {
                ...
                sscanf(reply2->c_str(), "%f %f %f %f %f %d %d %f %f %f %f %f %d %d %d %d %d %d %d %d %d %d %f %d %d", &grid_voltage_rating, &grid_current_rating, &out_voltage_rating, &out_freq_rating, &out_current_rating, &out_va_rating, &out_watt_rating, &batt_rating, &batt_recharge_voltage, &batt_under_voltage, &batt_bulk_voltage, &batt_float_voltage, &batt_type, &max_grid_charge_current, &max_charge_current, &in_voltage_range, &out_source_priority, &charger_source_priority, &max_parallel, &machine_type, &topology, &out_mode, &batt_redischarge_voltage, &pv_ok_condition, &pv_power_balance);
